...
class Parser {
  constructor (client, socket) {
    ...
    milo.setMode(this.parser, milo.MessageTypes.RESPONSE);

    // milo.setAfterStateChange(this.parser, () => {
    //   console.error(milo.States[milo.getState(this.parser)])
    //   return 0
    // })
    ...
    this[kMiloBuffer] = milo.alloc(65535);
  }
    ...
    try {
      ...
      if (errorCode !== milo.Errors.NONE) {
        ...
        const errorDescription = Buffer.from(
          ...
          milo.getErrorDescriptionLen(this.parser)
        ).toString('utf-8')
        ...
        */
        if (this[kMiloPausedByHandler] && milo.getState(this.parser) === milo.States.START) {
          this[kMiloPausedByHandler] = false
          milo.resume(this.parser)
  ...
  destroy () {
    ...
    this.paused = false
    if (this.parser) {
      milo.free(this[kMiloBuffer], 65535)
  ...
  onHeadersComplete () {
    ...
    const shouldKeepAlive = milo.getConnection(this.parser) === milo.Connections.KEEPALIVE
  ...
  onMessageComplete () {
    ...
    if (request.method !== 'HEAD' && milo.getRemainingContentLength(this.parser) > 0) {
  ...
  processOffsets () {
    const start = milo.getOffsets(this.parser)
    const flags = new Uint32Array(milo.memory.buffer, start, 3)
    const total = flags[2]
    const offsets = new Uint32Array(milo.memory.buffer, start + 12, total * 3)
    flags[2] = 0
    ...
    for (let i = 0; i < offsets.length / 3; i++) {
      ...
      switch (offsetType) {
        ...
        case milo.Offsets.MESSAGE_START:
          /* istanbul ignore next: difficult to make a test case for */
          if (this.socket.destroyed || !this.client[kQueue][this.client[kRunningIdx]]) {
            milo.fail(this.parser, milo.Errors.CALLBACK_ERROR, "Callback on_message_start failed with non zero return value.");
            return;
          }

          break
        case milo.Offsets.MESSAGE_COMPLETE:        
          /* istanbul ignore next: difficult to make a test case for */
          if (this.onMessageComplete() === false) {
            milo.fail(this.parser, milo.Errors.CALLBACK_ERROR, "Callback on_message_complete failed with non zero return value.");
            return;
          }
        ...
        case milo.Offsets.HEADER_VALUE:
          ...
          break
        case milo.Offsets.HEADERS:
          if(this.onHeadersComplete() === false) {
            milo.fail(this.parser, milo.Errors.CALLBACK_ERROR, "Callback on_headers failed with non zero return value.");
            return;
          } 

          break;
        case milo.Offsets.DATA:
          if(this.onData(offsetFrom, offsetSize) === false) {
            milo.fail(this.parser, milo.Errors.CALLBACK_ERROR, "Callback on_data failed with non zero return value.");
            return;
          }        
