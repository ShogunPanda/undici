...
class Parser {
  constructor (client, socket) {
    ...
    this.parser = milo.Parser.create()
    this.parser.mode = milo.RESPONSE

    this.parser.setOnMessageStart(this.onMessageStart.bind(this))
  ...
  onData (at, len) {
    ...
    try {
      if (request.onData(extractPayload(this.parser, at, len)) === false) {
        this.parser.pause()
        this[kMiloPausedByHandler] = true
      }
  ...
  processOffsets () {
    ...
    const offsets = this.parser.offsets
    ...
    for (let i = 1; i <= total; i++) {
      ...
      switch (offsetType) {
        case milo.Offsets.REASON:
          this.statusText = extractPayload(this.parser, offsetFrom, offsetSize).toString()
          break
        case milo.Offsets.HEADER_NAME:
        case milo.Offsets.HEADER_VALUE:
          {
            const data = extractPayload(this.parser, offsetFrom, offsetSize)
            this.headers.push(data)
            this.headersSize += offsetSize

            if (offsetType === milo.Offsets.HEADER_VALUE) {
          ...
          break
        case milo.Offsets.TRAILER_NAME:
        case milo.Offsets.TRAILER_VALUE:
          this.trailers.push(extractPayload(this.parser, offsetFrom, offsetSize))
