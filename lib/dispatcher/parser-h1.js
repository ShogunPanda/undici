...
class H1Parser {
  constructor (client, socket) {
    ...
    // Only enable the code below when debugging milo in undici locally
    // milo.setAfterStateChange(this.parser, (p, at, len) => {
    //   console.log(milo.getPosition(p), milo.States[milo.getState(p)], at, len)
    // })

    // milo.setOnError(this.parser, (p, at, len) => {
    //   console.log(milo.getPosition(p), milo.Errors[milo.getErrorCode(p)], milo.getErrorDescription(p))
    // })
  }
  ...
  execute (data) {
    ...
    if (data.length > this[kBufferSize]) {
      ...
      const newSize = Math.ceil(data.length / BUFFER_PAGE_SIZE) * BUFFER_PAGE_SIZE
    ...
    try {
      ...
      const unconsumedLength = this[kUnconsumedData].length
      if (unconsumedLength > 0) {
        this[kCurrentBuffer].set(this[kUnconsumedData])
        this[kUnconsumedData] = EMPTY_BUF
      }

      // Copy the data in the shared buffer.
      this[kCurrentBuffer].set(data, unconsumedLength)

      // Call the parse method in the WebAssembly layer
      const consumed = milo.parse(this.parser, this[kBuffer], unconsumedLength + data.length)
      ...
      if (errorCode !== milo.ERROR_NONE) {
        ...
        /*
          If the handler paused the parser at the end of a message, the parser will be paused forever.
          In that case let's resume.
        */
        if (milo.getState(this.parser) === milo.STATE_START) {
          milo.resume(this.parser)
          return
        }
  ...
  trackField (kind, at, len) {
    ...
    const data = len > 0 ? this.extractPayload(at, len) : Buffer.from('')
    this[kind].push(data)
  }
